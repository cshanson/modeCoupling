...
if 'cart_MC' not in locals():
	...
	# with NP.load(DATADIR + '/%s/Surface_Flow_Maps/SurfaceFlows_Bcoeffs.npz' % (subDir)) as DICT:
		# QX,QY,dkx,dky = [NP.array(DICT[x]) for x in ['QX','QY','dkx','dky']]
		# mask = DICT['mask']
		# inds_kx,inds_ky = NP.where(mask)
	with h5py.File(DATADIR + '/%s/Surface_Flow_Maps/SurfaceFlows_Bcoeffs.h5' % (subDir)) as h5f:
		QX,QY,dkx,dky = [NP.array(h5f[x]) for x in ['QX','QY','dkx','dky']]
...
kxm,kym,Kernels = cart_MC.compute_kernels_parallel([nn,nnp],None,mask,QX,QY,\
										Basis1D,absq_range=NP.array([0,250])/RSUN,reorg_k=False,multiplyH=False,nbProc=25,\
										scaleFields=2)



with h5py.File('/scratch/ch3246/Private/Mode_Coupling/SG_Inversions/Kernels/Kernels_%s_SOLA_SurfaceMaps.h5' % (BASIS),'w') as h5f:
